...
We add N^2 because at the beginning we pre-check the whole figure to
lower the number of trees we will draw through the dfs.

Why multipling by 2N:

2 copied lists

Space Complexity: O(N^4) = O(h * (2N^2) ) where h = N^2 - k
"""
import copy
...
class Solution(object):
    ...
    def dfs(self, idx, col, board, nset, lst):
        ...
        stack = [(idx, col, board, nset, lst)]

        while len(stack) > 0:

            idx, col, board, nset, lst = stack.pop()
            ...
            if board[idx][col] == '.':
                ...
                if col < 8:
                    stack.append(
                        (idx, col + 1, board, nset, lst))
                elif col == 8 and idx < 8:
                    stack.append(
                        (idx + 1, 0, board, nset, lst))
                else:
                    return lst
